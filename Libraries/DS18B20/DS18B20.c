    ...
    for (;;)
    {
        ...
        for (i = 0; i < DS18B20_Quantity(); i++)
        {
            ...
            if (DS18B20_GetTemperature(i, &temperature))
            {
                ...
                int len = snprintf(message, sizeof(message), "%d. ROM: %X%X%X%X%X%X%X%X Temp: %f\r\n", i, ROM_tmp[0], ROM_tmp[1], ROM_tmp[2], ROM_tmp[3], ROM_tmp[4], ROM_tmp[5], ROM_tmp[6], ROM_tmp[7], temperature);
